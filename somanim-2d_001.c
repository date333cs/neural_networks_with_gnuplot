/*
  Self-Organizing Map (2D):  A demo using C language and gnuplot !!!
...
write_data_anim_d2(){
  ...
  for (i=0; i<N_XUNITS; i++){
    for (j=0; j<N_YUNITS; j++){
      fprintf(gp, "%.5lf %.5lf %.5lf\n", RV[i][j][0], 0.8, RV[i][j][1]); 
      fprintf(gp, "%.5lf %.5lf  %.5lf\n", (double)i/(double)N_XUNITS+0.01,0.1, (double)j/(double)N_YUNITS+0.01); 
